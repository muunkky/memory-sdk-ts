...
import { APIPromise } from '@xtraceai/memory/core/api-promise';

import util from 'node:util';
import Xtraceai from '@xtraceai/memory';
import { APIUserAbortError } from '@xtraceai/memory';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['XTRACEAI_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Xtraceai) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['XTRACEAI_LOG'] = 'debug';
      const client = new Xtraceai({
        logger: logger,
        apiKey: 'My API Key',
        orgID: 'My Org ID',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['XTRACEAI_LOG'] = 'not a log level';
      const client = new Xtraceai({
        logger: logger,
        apiKey: 'My API Key',
        orgID: 'My Org ID',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'XTRACEAI_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['XTRACEAI_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['XTRACEAI_BASE_URL'] = 'https://example.com/from_env';
      const client = new Xtraceai({ apiKey: 'My API Key', orgID: 'My Org ID' });
    ...
    test('in request options overridden by env variable', () => {
      process.env['XTRACEAI_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Xtraceai({ apiKey: 'My API Key', orgID: 'My Org ID' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    ...
    // default
    const client2 = new Xtraceai({ apiKey: 'My API Key', orgID: 'My Org ID' });
